...
app.use(
  cors({
    origin: process.env.NODE_ENV === 'production' ? (process.env.ALLOWED_ORIGIN || devOrigin) : true,
    credentials: true,
  })
);
...
// In development, respond to root with a friendly message
if (process.env.NODE_ENV !== 'production') {
  app.get('/', (_req, res) => {
    res.status(200).json({
      message: 'AI server up. Use POST /api/AIChat. Health: /healthz',
    });
  });
}
...
app.post('/api/AIChat', async (req, res) => {
  try {
    ...
    const useMock = process.env.USE_MOCK_RESPONSE === 'true' || (!OPENAI_API_KEY && process.env.NODE_ENV !== 'production');

    // Mock responses in dev if requested or if key is missing
    if (useMock) {
      if (!OPENAI_API_KEY && process.env.NODE_ENV !== 'production') {
        console.warn('OPENAI_API_KEY missing; using mock responses in development');
      } else {
...
Remember: You are a helpful assistant that provides general health information, not a licensed medical professional.` 
    ...
    // In development, fall back to mock response instead of 500
    if (process.env.NODE_ENV !== 'production') {
      const q = typeof req?.body?.question === 'string' ? req.body.question : '';
      const mockAnswer = `Development fallback response.\n\nYour question: "${q}"\n\nThe AI service is unavailable in development. Returning a safe mock reply. For real responses, set OPENAI_API_KEY.`;
      return res.status(200).json({ answer: mockAnswer });
    }
    return res.status(500).json({
      error: `Server error making AI request`
    });
  }
});
